...
from actions.user_endpoint_actions import do_post_request_to_create_user
...
def test_user_can_be_created():
    ...
    response = do_post_request_to_create_user(valid_request_body)
    with soft_assertions():
        ...
        assert_that(response.json()) \
            ...
            .is_equal_to(valid_request_body, ignore="id")


@pytest.mark.backend
@pytest.mark.users
@pytest.mark.create_users
def test_several_users_cannot_be_added_with_same_email():
    valid_request_body = get_valid_create_user_payload()
    do_post_request_to_create_user(valid_request_body)
    same_email = valid_request_body['email']
    new_request_body = get_valid_create_user_payload()
    new_request_body['email'] = same_email
    response = do_post_request_to_create_user(new_request_body)
    with soft_assertions():
        assert_that(response.status_code) \
...
def test_cannot_add_user_without_param(param, reason):
    ...
    response = do_post_request_to_create_user(request_body)
    with soft_assertions():
        assert_that(response.status_code) \
