...
from actions.user_endpoint_actions import delete_request_for_user
...
@pytest.mark.users
@pytest.mark.delete_users
def test_cannot_delete_user_with_invalid_user_id(invalid_user_id):
    delete_response = delete_request_for_user(invalid_user_id)
    with soft_assertions():
        assert_that(delete_response.status_code) \
        ...
        assert_that(delete_response.json()) \
            ...
            .is_equal_to(f'User with id = {invalid_user_id} '
                         f'was not found')
